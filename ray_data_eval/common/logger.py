...
def plot_from_jsonl(file_path, output_path):
    ...
    data['memory_usage_in_MB'] = data['memory_usage_in_bytes'] / (1024 * 1024)
    ...
    for index, row in data.iterrows():
        current_sum += 1 if row['producer_finished'] or row['consumer_finished'] else 0
    ...
    ax2.plot(data['time'], data['memory_usage_in_MB'], color=color)
