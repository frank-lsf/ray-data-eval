...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dummy:
        ...
        train_partitioning = Partitioning(
            PartitionStyle.DIRECTORY, field_names=["category"], base_dir=traindir
        )
        ...
        val_partitioning = Partitioning(
            PartitionStyle.DIRECTORY, field_names=["category"], base_dir=valdir
        )
...
def train(train_dataset, model, criterion, optimizer, epoch, device, args):
    ...
    for batch in train_dataset.iter_torch_batches(
        batch_size=args.batch_size, collate_fn=_collate_fn_arr_pil_images_to_tensor
    ):
