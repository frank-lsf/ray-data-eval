...
producer_task_name = "task::ReadImage"
consumer_task_name = "task::MapBatches(encode_and_upload)"
...
def assign_slots(events, num_cpus, num_gpus):
    ...
    for event in events:
        ...
        for i in range(num_cpus + num_gpus):
            if (
                event["cat"] in [producer_task_name, consumer_task_name, write_task_name]
                and i > num_cpus
            ):
                continue
